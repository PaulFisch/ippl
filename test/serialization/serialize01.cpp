...
int main(int argc, char* argv[]) {
    ...
    Kokkos::initialize(argc, argv);
    {
        ...
        if (rank > 0) {
            auto h_id = Kokkos::create_mirror_view(bunch.id_m);
            Kokkos::deep_copy(h_id, bunch.id_m);

            auto h_charge = Kokkos::create_mirror_view(bunch.charge_m);
            Kokkos::deep_copy(h_charge, bunch.charge_m);

            auto h_mass = Kokkos::create_mirror_view(bunch.mass_m);
