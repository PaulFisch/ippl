...
namespace ippl {
    ...
    void ParticleSpatialLayout<T, Dim, Mesh, Properties...>::update(ParticleContainer& pc) {
        ...
        auto [nInvalid, nDestinationRanks] =
            locateParticles(pc, particleRanks, rankSendCount_dview, destinationRanks_dview);
        ...
        // Reconstruct the bool validity array locally from particleRanks.
        // Kept out of locateParticles to minimize peak memory overlap with sortedIds above.
        auto myRank = Comm->rank();
        pc.internalDestroy(
            KOKKOS_LAMBDA(size_t i) {
                return particleRanks(i) != myRank;
            },
            nInvalid);
        Kokkos::fence();
    ...
    ParticleSpatialLayout<T, Dim, Mesh, Properties...>::locateParticles(
        ...
        Kokkos::parallel_reduce(
            ...
            Kokkos::RangePolicy<size_t>(0, pc.getLocalNum()),
