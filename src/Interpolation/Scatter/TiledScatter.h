...
namespace ippl::Interpolation::detail {
    ...
    template <int W, class Types, bool = true>
    struct TiledScatter {
        static constexpr bool requires_binning = true;
        static constexpr bool UseSorting       = true;
        ...
        using scratch_view    = Kokkos::View<RealType*, scratch_space,
        ...
        // Local histogram with type-aware atomic operations
        template <typename GridValueT>
        struct Histogram {
            static constexpr bool is_complex = std::is_same_v<GridValueT, Kokkos::complex<RealType>>;

            scratch_view data_r;
            scratch_view data_i;  // unused if !is_complex
            Vector<int, Dim> size;
            size_t total;

            KOKKOS_INLINE_FUNCTION void init(const team_member& team, scratch_view r, scratch_view i,
                                             Vector<int, Dim> sz, size_t n) {
                data_r = r;
                ...
                Kokkos::parallel_for(Kokkos::TeamThreadRange(team, total), [&](size_t idx) {
                    data_r(idx) = 0;
                    if constexpr (is_complex) data_i(idx) = 0;
                });
            }
            ...
            KOKKOS_INLINE_FUNCTION bool in_bounds(const Kokkos::Array<int, Dim>& c) const {
                for (unsigned d = 0; d < Dim; ++d)
                    if (c[d] < 0 || c[d] >= size[d]) return false;
                return true;
            }

            template <typename V>
            KOKKOS_INLINE_FUNCTION void atomic_add(const Kokkos::Array<int, Dim>& c, V val, RealType w) {
                if (!in_bounds(c)) return;
                const size_t idx = to_flat(c);

                if constexpr (is_complex && std::is_same_v<V, Kokkos::complex<RealType>>) {
                    Kokkos::atomic_add(&data_r(idx), val.real() * w);
                    Kokkos::atomic_add(&data_i(idx), val.imag() * w);
                } else if constexpr (is_complex) {
                    Kokkos::atomic_add(&data_r(idx), val * w);
                } else {
                    Kokkos::atomic_add(&data_r(idx), val * w);
                }
            }
        ...
        KOKKOS_INLINE_FUNCTION void operator()(const team_member& team) const {
            ...
            // Scatter particles to histogram
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, total), [&](size_t idx) {
                ...
                auto write = [&]<unsigned D>(auto&& self, auto... is) {
                    if constexpr (D == 0) {
                        Kokkos::atomic_add(&args.grid(gc[0], is...),
                                           static_cast<grid_value_t>(hist(idx)));
                    } else {
                        self.template operator()<D - 1>(self, gc[D], is...);
                    }
                };
                write.template operator()<Dim - 1>(write);
            });
        }
        ...
        void run(size_t) {
            ...
            Kokkos::parallel_for("TiledScatter",
