...
// Bank-conflict avoidance
// -----------------------
// GPU shared memory has 32 banks of 4-byte words (bank = word_address % 32).
// A bank conflict occurs when multiple lanes in the SAME WARP INSTRUCTION
// access different words in the SAME bank — successive iterations are separate
// instructions and do NOT conflict.
//
//  Scatter phase (2D/3D): warp lanes cover ONLY the x-dimension (i0).
//    Lane l reads/writes hist[ base_row + l ] → consecutive addresses →
//    consecutive banks → zero conflicts.
//    The outer loops over i1 / i2 run serially per lane, so no cross-row
//    bank conflicts arise within one instruction.
//
//  Scatter phase (1D): same — one ThreadVectorRange over W x-offsets.
//
//  Reduction phase (inner loop over v):
//    Lane l reads  hist[ v*stride + row_base + l ]  for v = 0..nv-1.
//    Within one v, 32 consecutive addresses → 32 consecutive banks → OK.
//    The stride between vector copies is `hist_stride` (padded in 32-bit words).
//    For double (2 words/element): conflict condition is stride*2 % 32 == 0,
//    i.e. stride % 16 == 0.  The word-aware padded_stride() below handles this.
//
//  Integer instruction count
//  -------------------------
//  •  All local / shared-memory indices are 32-bit (int) — no 64-bit IMAD.
//  •  Row and slab base offsets are precomputed once per outer-loop iteration,
//     not recomputed per lane.
//  •  The flat→multidim decode in the reduction is replaced by a natural
//     multidimensional TeamThreadRange × ThreadVectorRange nest; no % or /.
// ============================================================================
...
namespace ippl::Interpolation::detail {
    ...
    struct GridParallelScatter {
        ...
        // ── Bank-conflict-safe padding (word-aware) ──────────────────────────
        // Banks are 32-bit words.  For an element type of `elem_words` words,
        // a conflict in the reduction loop arises when
        //   (stride * elem_words) % 32 == 0
        // i.e. all nv copies land in the same bank for the same logical entry.
        // We increment stride until this condition is broken.
        //
        // float : elem_words = 1  → avoid stride % 32 == 0
        // double: elem_words = 2  → avoid stride % 16 == 0
        static constexpr int kBankWords  = 32;
        static constexpr int kElemWords  = static_cast<int>(sizeof(RealType) / 4);

        KOKKOS_INLINE_FUNCTION
        static int padded_stride(int htot) noexcept {
            int s = htot;
            while (((s * kElemWords) % kBankWords) == 0) ++s;
            return s;
        }
        ...
        //  [B]  nv * hist_stride   hist_i  (imag, complex only)
        //  [C]  nv * Dim * W       kw      (kernel weights per vec)
        //  [D]  nv * Dim           base_s  (stencil bases per vec, int)
        //
        template <bool IsComplex>
        static size_t compute_scratch_size(const Vector<int, Dim>& tile_size, int team_size,
                                           int z_batches = 1) {
            ...
            const int nv     = std::max(1, team_size);
            const int stride = padded_stride(htot);
        ...
        // Set in run(), read in operator()
        int    sub_teams_per_tile_ = 1;
        int    hist_stride_        = 1;   // padded stride between per-vector copies (in elements)
        ...
        KOKKOS_INLINE_FUNCTION int hist_total() const {
            int n = 1;
            for (unsigned d = 0; d < Dim; ++d)
                n *= args.tile_size[d] + ((Dim == 3 && d == 2) ? z_batch_size_ : W) + 1;
            return n;
        }

        KOKKOS_INLINE_FUNCTION Vector<int, Dim> decode_tile_base(int tile_id) const {
            Vector<int, Dim> tile_base;
            for (int t = tile_id, d = static_cast<int>(Dim); d-- > 0;) {
                tile_base[d] = (t % args.num_tiles[d]) * args.tile_size[d];
                t /= args.num_tiles[d];
            }
            return tile_base;
        }
        ...
        KOKKOS_INLINE_FUNCTION void operator()(const team_member& team) const {
            ...
            // ── Tile / sub-team bookkeeping ──────────────────────────────────
            // Use int arithmetic where safe; only bin_start/bin_end/pstart/pend
            // remain size_t (they index global particle arrays).
            const int    league_r  = team.league_rank();
            const int    tile_id   = league_r / sub_teams_per_tile_;
            const int    sub_id    = league_r % sub_teams_per_tile_;
            const size_t bin_start = args.bin_offsets(tile_id);
            const size_t bin_end   = args.bin_offsets(tile_id + 1);
            const size_t bin_size  = bin_end - bin_start;

            const size_t particles_per_sub =
                (bin_size + static_cast<size_t>(sub_teams_per_tile_) - 1)
                / static_cast<size_t>(sub_teams_per_tile_);
            const size_t pstart = bin_start + static_cast<size_t>(sub_id) * particles_per_sub;
            if (pstart >= bin_end)
                return;
            const size_t pend = Kokkos::min(bin_end, pstart + particles_per_sub);

            const auto tile_base = decode_tile_base(tile_id);
            const auto hs        = hist_size();     // int per dim
            const int  htot      = hist_total();    // int — fits in shmem
            const int  stride    = hist_stride_;    // padded stride (int)

            // Hoist frequently used invariants into registers
            const int hs0  = hs[0];
            const int hs1  = (Dim >= 2) ? hs[1] : 1;
            const int hs01 = hs0 * hs1;
            const int ngh  = args.nghost;
            ...
            // ── Zero this warp's histogram slice ─────────────────────────────
            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, htot), [&](int i) {
                hist_r(vec_id * stride + i) = RealType(0);
                if constexpr (gcplx)
                    hist_i(vec_id * stride + i) = RealType(0);
            });
            ...
            for (size_t ip = pstart + static_cast<size_t>(vec_id); ip < pend;
                ...
                // ── Step 1: kernel weights ────────────────────────────────────
                // Lane `flat` computes weight for dim d = flat/W, offset i = flat%W.
                Kokkos::parallel_for(
                    Kokkos::ThreadVectorRange(team, static_cast<int>(Dim) * W),
                    [&](int flat) {
                        const int      d    = flat / W;
                        ...
                        const RealType gp   = transform.toGridCoordinate(args.x(p)[d], d);
                        const int      idx0 = transform.getStencilBase(gp - RealType(0.5), W);
                        my_kw[d * W + i]    = args.kernel(
                            (gp - (RealType(idx0 + i) + RealType(0.5))) * args.inv_hw);
                        if (i == 0)
                            my_base[d] = idx0 - args.local_offset[d];
                    });
                ...
                //   We decompose the W^Dim stencil into outer loops over slower
                //   dimensions and an inner ThreadVectorRange over the x-dimension
                //   only.  This means EVERY warp instruction accesses W consecutive
                //   x-entries in shared memory → consecutive banks → zero conflicts.
                //   The old flat ThreadVectorRange(W*W) or (W*W*z) caused the warp
                //   to wrap across row/slab boundaries in a single instruction,
                //   creating non-unit-stride accesses and real bank conflicts.
                //
                //   Row/slab base offsets are computed once per outer iteration,
                //   not per lane, saving repeated multiply/add in the hot path.
                ...
                if constexpr (Dim == 1) {
                    // ---------------------------------------------------------
                    // 1D: single ThreadVectorRange over W x-offsets.
                    // ---------------------------------------------------------
                    const int bh0 = my_base[0] + half_left - tile_base[0];

                    Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, W), [&](int i0) {
                        const int      hidx = bh0 + i0;
                        const RealType w    = my_kw[i0];
                        h_r[hidx] += val_r * w;
                        if constexpr (gcplx)
                            h_i[hidx] += val_i * w;
                    ...
                    // ---------------------------------------------------------
                    // 2D: serial loop over i1 (y), vectorised over i0 (x).
                    //
                    // Each ThreadVectorRange instruction covers one x-row:
                    //   h_r[ row_base + 0 ], h_r[ row_base + 1 ], ..., [ +W-1 ]
                    // These are W consecutive addresses → W consecutive banks.
                    // ---------------------------------------------------------
                    const int bh0 = my_base[0] + half_left - tile_base[0];
                    const int bh1 = my_base[1] + half_left - tile_base[1];
                    ...
                    for (int i1 = 0; i1 < W; ++i1) {
                        const RealType wy       = my_kw[W + i1];
                        const int      row_base = hs0 * (bh1 + i1) + bh0;

                        Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, W), [&](int i0) {
                            const int      hidx = row_base + i0;
                            const RealType w    = my_kw[i0] * wy;
                            h_r[hidx] += val_r * w;
                            if constexpr (gcplx)
                                h_i[hidx] += val_i * w;
                        });
                    ...
                    // ---------------------------------------------------------
                    // 3D: serial loops over i2 (z) and i1 (y), vectorised over
                    //     i0 (x).
                    //
                    // Slab base and row base are computed once per outer iteration.
                    // Each ThreadVectorRange covers one contiguous x-row.
                    // When z_batches > 1, only z-indices in [z_start_, z_end_)
                    // are processed; the histogram z-index is i2 - z_start_.
                    // ---------------------------------------------------------
                    const int bh0 = my_base[0] + half_left - tile_base[0];
                    const int bh1 = my_base[1] + half_left - tile_base[1];
                    const int bh2 = my_base[2] + half_left - tile_base[2];
                    ...
                    #pragma unroll
                    for (int i2 = z_start_; i2 < z_end_; ++i2) {
                        const RealType wz        = my_kw[2 * W + i2];
                        const int      slab_base = hs01 * (bh2 + i2 - z_start_);

                        #pragma unroll
                        for (int i1 = 0; i1 < W; ++i1) {
                            const RealType wyz      = my_kw[W + i1] * wz;
                            const int      row_base = slab_base + hs0 * (bh1 + i1) + bh0;

                            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, W),
                                                 [&](int i0) {
                                const int      hidx = row_base + i0;
                                const RealType w    = my_kw[i0] * wyz;
                                h_r[hidx] += val_r * w;
                                if constexpr (gcplx)
                                    h_i[hidx] += val_i * w;
                            });
            ...
            //
            // KEY CHANGE vs previous version:
            //   Instead of a flat TeamThreadRange(chunks) × ThreadVectorRange(VL)
            //   loop that decodes each flat index with expensive % and / operations,
            //   we iterate over the histogram in its natural multidimensional shape:
            //
            //     1D: ThreadVectorRange over hs0              (no outer loop)
            //     2D: TeamThreadRange over hs1, ThreadVectorRange over hs0
            //     3D: TeamThreadRange over hs2*hs1, ThreadVectorRange over hs0
            //
            //   For the y and z loops we use subtraction instead of % where possible
            //   (z = yzflat / hs1,  y = yzflat - z * hs1  avoids a second division).
            //
            //   Grid coordinates are derived directly from loop variables;
            //   no flat→multidim decode is needed inside the hot inner loop.
            //
            //   The inner ThreadVectorRange still covers W (or hs0) consecutive
            //   x-entries, so shared-memory access remains bank-conflict free.

            if constexpr (Dim == 1) {
                // -------------------------------------------------------------
                // 1D reduction: one ThreadVectorRange covers the whole histogram.
                // -------------------------------------------------------------
                Kokkos::parallel_for(
                    Kokkos::ThreadVectorRange(team, hs0), [&](int x) {
                        RealType sum_r = RealType(0);
                        RealType sum_i = RealType(0);
                        #pragma unroll
                        for (int v = 0; v < nv; ++v) {
                            sum_r += hist_r(v * stride + x);
                            if constexpr (gcplx)
                                sum_i += hist_i(v * stride + x);
                        }

                        const int local = tile_base[0] + x - half_left;
                        if (local < -ngh || local >= args.n_grid_local[0] + ngh)
                            return;
                        const int gx = local + ngh;

                        if constexpr (gcplx) {
                            RealType* ptr = reinterpret_cast<RealType*>(&args.grid(gx));
                            Kokkos::atomic_add(&ptr[0], sum_r);
                            Kokkos::atomic_add(&ptr[1], sum_i);
                        } else {
                            using gv = grid_value_t;
                            Kokkos::atomic_add(&args.grid(gx), static_cast<gv>(sum_r));
                        }
                    });

            } else if constexpr (Dim == 2) {
                // -------------------------------------------------------------
                // 2D reduction: TeamThreadRange over y-rows, ThreadVectorRange
                //               over x-entries within each row.
                // -------------------------------------------------------------
                Kokkos::parallel_for(
                    Kokkos::TeamThreadRange(team, hs1), [&](int y) {
                        const int row_base = hs0 * y;
                        ...
                        Kokkos::parallel_for(
                            Kokkos::ThreadVectorRange(team, hs0), [&](int x) {
                                ...
                                const int lx = tile_base[0] + x - half_left;
                                const int ly = tile_base[1] + y - half_left;
                                if (lx < -ngh || lx >= args.n_grid_local[0] + ngh) return;
                                if (ly < -ngh || ly >= args.n_grid_local[1] + ngh) return;
                                const int gx = lx + ngh;
                                const int gy = ly + ngh;

                                if constexpr (gcplx) {
                                    RealType* ptr =
                                        reinterpret_cast<RealType*>(&args.grid(gx, gy));
                                    Kokkos::atomic_add(&ptr[0], sum_r);
                                    Kokkos::atomic_add(&ptr[1], sum_i);
                                } else {
                                    using gv = grid_value_t;
                                    Kokkos::atomic_add(&args.grid(gx, gy),
                                                       static_cast<gv>(sum_r));
                                }
                            });
                    });
                ...
                // -------------------------------------------------------------
                // 3D reduction: TeamThreadRange over (z*hs1 + y) flattened pairs,
                //               ThreadVectorRange over x.
                //
                //   yzflat ∈ [0, hs2*hs1)
                //   z      = yzflat / hs1    (one integer division per team-thread)
                //   y      = yzflat - z*hs1  (subtraction instead of %)
                //
                //   Histogram z-index is the loop variable; the corresponding
                //   global z = tile_base[2] + (z_hist_idx + z_start_) - half_left.
                // -------------------------------------------------------------
                const int hs2    = hs[2];
                const int yz_tot = hs2 * hs1;

                Kokkos::parallel_for(
                    Kokkos::TeamThreadRange(team, yz_tot), [&](int yzflat) {
                        const int z_hist = yzflat / hs1;
                        const int y      = yzflat - z_hist * hs1;
                        const int slab_base = hs01 * z_hist + hs0 * y;

                        Kokkos::parallel_for(
                            Kokkos::ThreadVectorRange(team, hs0), [&](int x) {
                                const int idx = slab_base + x;
                                ...
                                // z_hist is the histogram's z coordinate;
                                // add z_start_ to recover the stencil-relative z.
                                const int lx = tile_base[0] + x             - half_left;
                                const int ly = tile_base[1] + y             - half_left;
                                const int lz = tile_base[2] + (z_hist + z_start_) - half_left;
                                if (lx < -ngh || lx >= args.n_grid_local[0] + ngh) return;
                                if (ly < -ngh || ly >= args.n_grid_local[1] + ngh) return;
                                if (lz < -ngh || lz >= args.n_grid_local[2] + ngh) return;
                                const int gx = lx + ngh;
                                const int gy = ly + ngh;
                                const int gz = lz + ngh;
                                ...
                                if constexpr (gcplx) {
                                    RealType* ptr =
                                        reinterpret_cast<RealType*>(&args.grid(gx, gy, gz));
                                    Kokkos::atomic_add(&ptr[0], sum_r);
                                    Kokkos::atomic_add(&ptr[1], sum_i);
                                } else {
                                    using gv = grid_value_t;
                                    Kokkos::atomic_add(&args.grid(gx, gy, gz),
                                                       static_cast<gv>(sum_r));
                                }
                            });
                    });
        ...
        void run(size_t n_particles) {
            ...
            int n_tiles = 1;
            for (unsigned d = 0; d < Dim; ++d)
                n_tiles *= args.num_tiles[d];

            if (n_tiles == 0 || n_particles == 0)
                return;

            sub_teams_per_tile_ = std::max(1, args.oversubscription_factor);

            const int z_batches = (Dim == 3) ? std::max(1, args.z_batches) : 1;
            z_batch_size_       = (W + z_batches - 1) / z_batches;

            // Compute padded histogram stride (word-aware, see padded_stride())
            int htot = 1;
            for (unsigned d = 0; d < Dim; ++d) {
                const int hist_dim = args.tile_size[d]
                                     + ((Dim == 3 && d == 2) ? z_batch_size_ : W) + 1;
            ...
            for (int batch = 0; batch < z_batches; ++batch) {
                ...
                Kokkos::parallel_for(
                    "GridParallelScatterVectorized",
                    team_policy(static_cast<int>(n_tiles) * sub_teams_per_tile_,
                                args.team_size, vector_length)
                        .set_scratch_size(0, Kokkos::PerTeam(scratch)),
                    *this);
            }
        }
