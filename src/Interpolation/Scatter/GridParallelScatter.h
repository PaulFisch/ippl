...
namespace ippl::Interpolation::detail {
    ...
    struct GridParallelScatter {
        ...
        // ── Scratch layout (per team) ────────────────────────────────────────
        //
        //  [A]  nv * htot   hist_r  (real part of private per-lane histograms)
        //  [B]  nv * htot   hist_i  (imag, complex grids only)
        //  [C]  nv * Dim*W  kw      (kernel weights [lane][d][wi])
        //  [D]  nv * Dim    base_s  (per-lane stencil bases relative to tile)
        //
        //  htot uses a reduced z-extent when z_batches > 1.
        // ─────────────────────────────────────────────────────────────────────
        template <bool IsComplex>
        static size_t compute_scratch_size(const Vector<int, Dim>& tile_size, int team_size,
                                           int z_batches = 1) {
        ...
        // Z-batching state (set once per batch in run(), read in operator())
        int z_batch_size_ = W;
        int z_start_      = 0;
        int z_end_        = W;
        ...
        KOKKOS_INLINE_FUNCTION void operator()(const team_member& team) const {
            ...
            // Private pointers into this lane's scratch slices.
            // Indexing is always by a compile-time-bounded value (d<Dim, wi<W) so
            // the compiler can keep these in registers rather than local memory.
            RealType* my_kw = kw.data() + vec_id * (static_cast<int>(Dim) * W);
            int* my_base    = base_s.data() + vec_id * static_cast<int>(Dim);

            // ── Zero this lane's histogram slice ──────────────────────────────
            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, htot), [&](size_t i) {
                hist_r(vec_id * htot + i) = RealType(0);
                if constexpr (gcplx)
                    hist_i(vec_id * htot + i) = RealType(0);
            ...
            for (size_t ip = pstart + static_cast<size_t>(vec_id); ip < pend;
                ...
                // ── Kernel weights + stencil bases ────────────────────────────
                //
                // Unrolled over d*W cells; the loop body touches only shared
                // memory (my_kw, my_base) and register scalars.  No local arrays.
                Kokkos::parallel_for(
                    Kokkos::ThreadVectorRange(team, static_cast<int>(Dim) * W), [&](int flat) {
                        ...
                        if (wi == 0)
                            my_base[d] = idx0 - args.local_offset[d] + half_left - tile_base[d];
                    });

                // ── Scatter to private histogram ──────────────────────────────
                //
                // Key reduction in integer work vs the original:
                //   Precompute base_hidx from my_base[] once per particle.
                //   The hot-loop body then only adds stencil-local offsets whose
                //   strides (1, hs[0], hs[0]*hs[1]) are loop-invariant, so the
                //   compiler can strength-reduce the multiplies to additions.
                RealType* h_r = hist_r.data() + vec_id * htot;
                RealType* h_i = gcplx ? hist_i.data() + vec_id * htot : nullptr;

                if constexpr (Dim == 1) {
                    const size_t base_hidx = static_cast<size_t>(my_base[0]);

                    Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, W), [&](int wi) {
                        const size_t hidx = base_hidx + static_cast<size_t>(wi);
                        const RealType w  = my_kw[wi];
                        h_r[hidx] += val_r * w;
                        if constexpr (gcplx)
                            h_i[hidx] += val_i * w;
                    });
                    ...
                    // base_hidx folds both particle-dependent offsets;
                    // only i0 + hs[0]*i1 (loop-local, bounded by W²) remains inside.
                    const size_t base_hidx =
                        static_cast<size_t>(my_base[0])
                        + static_cast<size_t>(hs[0]) * static_cast<size_t>(my_base[1]);

                    Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, W * W), [&](int flat) {
                        const int i0      = flat % W;
                        const int i1      = flat / W;
                        const size_t hidx = base_hidx + static_cast<size_t>(i0)
                    ...
                    // Fold all three particle-dependent offsets into base_hidx.
                    // z_start_ is subtracted here because the histogram z-extent
                    // is only z_batch_size_ wide; z is stored relative to z_start_.
                    const size_t base_hidx =
                        static_cast<size_t>(my_base[0])
                        + static_cast<size_t>(hs[0])
                              * (static_cast<size_t>(my_base[1])
                                 + static_cast<size_t>(hs[1])
                                       * static_cast<size_t>(my_base[2] - z_start_));

                    const int z_range = z_end_ - z_start_;
                    Kokkos::parallel_for(
                        Kokkos::ThreadVectorRange(team, W * W * z_range), [&](int flat) {
                            const int i0 = flat % W;
                            const int i1 = (flat / W) % W;
                            const int i2 = flat / (W * W);  // 0-relative within batch
                            const size_t hidx =
                                base_hidx + static_cast<size_t>(i0)
                                + static_cast<size_t>(hs[0])
                                      * (static_cast<size_t>(i1)
                                         + static_cast<size_t>(hs[1]) * static_cast<size_t>(i2));
                            const RealType w =
                                my_kw[i0] * my_kw[W + i1] * my_kw[2 * W + i2 + z_start_];
                            h_r[hidx] += val_r * w;
                            if constexpr (gcplx)
                                h_i[hidx] += val_i * w;
                        });
            ...
            // ── Reduce nv private histograms → global grid ────────────────────
            const size_t chunks = (htot + static_cast<size_t>(vector_length) - 1)
                                  / static_cast<size_t>(vector_length);

            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, chunks), [&](size_t chunk) {
                Kokkos::parallel_for(Kokkos::ThreadVectorRange(team, vector_length), [&](int lane) {
                    const size_t idx =
                        chunk * static_cast<size_t>(vector_length) + static_cast<size_t>(lane);
                    if (idx >= htot)
                        return;
                    ...
                    // Decode flat histogram index → per-dim histogram coords
                    size_t tmp = idx;
                    Kokkos::Array<int, Dim> hc{};
                    for (unsigned d = 0; d < Dim; ++d) {
                        hc[d] = static_cast<int>(tmp % static_cast<size_t>(hs[d]));
        ...
        void run(size_t n_particles) {
            ...
            hist_total_ = 1;
            for (unsigned d = 0; d < Dim; ++d) {
                const int ext = args.tile_size[d] + ((Dim == 3 && d == 2) ? z_batch_size_ : W) + 1;
                hist_total_ *= static_cast<size_t>(ext);
            }
