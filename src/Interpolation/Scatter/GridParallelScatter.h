...
//             const int pitch1, const RealType* __restrict__ kw, const RealType vr, const RealType
//             vi, RealType* __restrict__ local_r, RealType* __restrict__ local_i) { const RealType*
//             __restrict__ kwx = kw; const RealType* __restrict__ kwy = kw + W; const RealType*
//             __restrict__ kwz = kw + 2 * W;
//
...
//                                                Kokkos::max<size_t>(1, (bin_size + tgt - 1) /
//                                                tgt));
...
//                         kw[wi]         = args.kernel((gp0 - (RealType(idx0 + wi) +
//                         RealType(0.5)))
//                                                      * args.inv_hw);
//                         kw[W + wi]     = args.kernel((gp1 - (RealType(idx1 + wi) +
//                         RealType(0.5)))
//                                                      * args.inv_hw);
//                         kw[2 * W + wi] = args.kernel((gp2 - (RealType(idx2 + wi) +
//                         RealType(0.5)))
//                                                      * args.inv_hw);
//                     }
//
...
//             const size_t* __restrict__ stride, const RealType* __restrict__ kw, const RealType
//             vr, const RealType vi, RealType* __restrict__ local_r, RealType* __restrict__
...
namespace ippl::Interpolation::detail {
    ...
    struct GridParallelScatterImpl3D {
        ...
        // Simple, tight stride for 1D evaluated weights
        static constexpr int ker_stride = 3 * W;
        ...
        KOKKOS_FORCEINLINE_FUNCTION static void scatter_particle_fast(
            ...
            // Flat loop directly matching CUDA algorithm: idx < W*W*W
            // No inner barriers, no slice serialization.
            for (int idx = team.team_rank(); idx < stencil_total; idx += team.team_size()) {
                const int zz  = idx / plane;
                const int rem = idx - zz * plane;
                const int yy  = rem / W;
        ...
        KOKKOS_INLINE_FUNCTION void operator()(const team_member& team) const {
            ...
            // Raw unpadded array sizes
            const int hs0 = args.tile_size[0] + padded_extra;
            const int hs1 = args.tile_size[1] + padded_extra;

            const size_t pitch0 = static_cast<size_t>(hs0);
            const size_t pitch1 = pitch0 * static_cast<size_t>(hs1);
...
#pragma unroll
                ...
                // Sequential over batch, parallel over stencil (exactly matching CUDA loop)
                for (int bi = 0; bi < batch_size; ++bi) {
                    const int sx             = shifts_x[bi];
                    const int sy             = shifts_y[bi];
                    const int sz             = shifts_z[bi];
                    ...
                    scatter_particle_fast<needs_imag>(team, sx, sy, sz, pitch0, pitch1, kw, vr, vi,
                                                      local_r, local_i);
                    team.team_barrier();  // Synchronize after EACH particle is scattered
                }
            }

            // Flat flush loop, matching your `for (int n = threadIdx.x; n < local_subgrid.size();
            // ...)`
            for (size_t n = team.team_rank(); n < phys_total; n += team.team_size()) {
                const int kz  = n / pitch1;
                const int rem = n - kz * pitch1;
                const int jy  = rem / pitch0;
                const int ix  = rem - jy * pitch0;

                const RealType rr = local_r[n];
                if constexpr (needs_imag) {
                    if (rr == RealType(0) && local_i[n] == RealType(0))
                        continue;
                } else {
                    if (rr == RealType(0))
                        continue;
                ...
                if (local_x >= -args.nghost && local_x < args.n_grid_local[0] + args.nghost
                    ...
                    if constexpr (grid_complex) {
                        ...
                        Kokkos::atomic_add(&ptr[0], rr);
                        if constexpr (needs_imag)
                            Kokkos::atomic_add(&ptr[1], local_i[n]);
                    } else {
                        Kokkos::atomic_add(&args.grid(gx, gy, gz), static_cast<grid_value_t>(rr));
                    }
                }
        ...
        static size_t compute_scratch_size(const Vector<int, Dim>& tile_size, int /*team_size*/,
            ...
            const size_t phys_total = static_cast<size_t>(hs0) * hs1 * hs2;
        ...
        void run(size_t n_particles) {
            ...
            physical_total_ = static_cast<size_t>(hs0) * hs1 * hs2;
    ...
    struct GridParallelScatterImplND {
        ...
        KOKKOS_INLINE_FUNCTION void operator()(const team_member& team) const {
            ...
            for (size_t lid = static_cast<size_t>(team.team_rank()); lid < phys_total;
                 lid += static_cast<size_t>(team.team_size())) {
                size_t tmp = lid;
                int coord[Dim];
                size_t sidx = 0;
        ...
        static size_t compute_scratch_size(const Vector<int, Dim>& tile_size, int /*team_size*/,
            ...
            size_t phys_total = 1;
            for (unsigned d = 0; d < Dim; ++d) {
                phys_total *= static_cast<size_t>(tile_size[d] + padded_extra);
            }
        ...
        void run(size_t n_particles) {
            ...
            physical_total_ = 1;
            for (unsigned d = 0; d < Dim; ++d) {
                physical_total_ *= static_cast<size_t>(args.tile_size[d] + padded_extra);
            }
