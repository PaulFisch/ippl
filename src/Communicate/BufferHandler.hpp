...
namespace ippl {
    ...
    DefaultBufferHandler<MemorySpace>::reallocateLargestFreeBuffer(size_type requiredSize) {
        // Allocate a fresh buffer instead of reallocating the largest free
        // one. A free + alloc cycle can release the device pointer that a
        // GPU-aware MPI's registration cache still holds, and a subsequent
        // alloc returning the same virtual address would surface a stale IPC
        // handle on the next transfer. Keeping old buffers in the free pool
        // and returning a new buffer at a new address sidesteps this.
        return allocateNewBuffer(requiredSize);
    }
