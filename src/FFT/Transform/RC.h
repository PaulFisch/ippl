#ifndef IPPL_FFT_TRANSFORM_RC_HPP
#define IPPL_FFT_TRANSFORM_RC_HPP
...
public:
    ...
    using ComplexField = Field<Complex_t, Dim,
    ...
    FFT(const Layout_t& layoutIn, const Layout_t& layoutOut, const ParameterList& params) {
        ...
        backend_ = std::make_unique<Backend_t>(
            heffte::box3d<long long>{lowIn, highIn},
    ...
    void transform(TransformDirection direction, RealField& f, ComplexField& g) {
        ...
        fft::copyToTemp<ExecSpace, decltype(tempComplex_), decltype(gview)>(tempComplex_, gview, ngg);
        ...
        fft::copyFromTemp<ExecSpace, decltype(fview), decltype(tempReal_)>(fview, tempReal_, ngf);
        fft::copyFromTemp<ExecSpace, decltype(gview), decltype(tempComplex_)>(gview, tempComplex_, ngg);
    }
