...
//         //=========================================================================
//         // Backend Traits for CuFFTC2C
//         //=========================================================================
//
//         template <typename T, unsigned Dim, typename MemSpace>
//         struct backend_traits<CuFFTC2C<T, Dim, MemSpace>> {
//             // FIX 1: Standard CuFFTC2C does not support batched 3D natively without
//             cufftPlanMany.
...
// #endif
//         template <typename Backend>
//         inline constexpr bool supports_native_batched_v =
//             backend_traits<Backend>::supports_native_batched;
//
//     }  // namespace fft
//
//     //=========================================================================
...
//
//         FFT(const Layout_t& layoutIn, const Layout_t& layoutOut, const PruningParams<Dim>&
//         pruning,
...
//
//             auto& prunedLayout =
//                 (layoutOut.getLocalNDIndex().size() < layoutIn.getLocalNDIndex().size()) ?
//                 layoutOut
//                                                                                          :
//                                                                                          layoutIn;
//
...
//                 numConcurrent_ =
//                     std::clamp(params.get<int>("num_concurrent_ffts", 4), 1, NumSubFFTs);
...
//                     {0, 0, 0, 0}, {NumSubFFTs, long(owned[0].length()), long(owned[1].length()),
//                                    long(owned[2].length())}),
...
//                         size_t idx =
//                             (i - ngOut)
//                             + owned[0].length() * ((j - ngOut) + owned[1].length() * (kk -
//                             ngOut));
...
//                     temps_[s] =
//                         detail::shrinkView("pruned_temp_" + std::to_string(s), outView, ngOut);
//                 }
...
//
//                     static IpplTimings::TimerRef heffteTimer =
//                     IpplTimings::getTimer("RunHeffte"); IpplTimings::startTimer(heffteTimer); if
//                     (dir == 1) {
...
//
//                             int64_t f0 =
//                                 (gi < int64_t(modes[0]) / 2)
//                                     ? gi
//                                     : int64_t(gDomFull[0].length()) - int64_t(modes[0]) + gi;
//                             int64_t f1 =
//                                 (gj < int64_t(modes[1]) / 2)
//                                     ? gj
//                                     : int64_t(gDomFull[1].length()) - int64_t(modes[1]) + gj;
//                             int64_t f2 =
//                                 (gk < int64_t(modes[2]) / 2)
//                                     ? gk
//                                     : int64_t(gDomFull[2].length()) - int64_t(modes[2]) + gk;
//
//                             Complex_t w(1.0, 0.0);
...
//                     temps_[s] =
//                         detail::shrinkView("pruned_ifft_temp_" + std::to_string(s), inView,
//                         ngIn);
//                 }
//             }
//
...
#pragma omp parallel for
    ...
    void FFT<PrunedCCTransform, ComplexField>::backwardPruned(int dir, ComplexField& input,
        ...
        // Ensure temps (sized for pruned domain)
        for (int s = 0; s < numConcurrent_; ++s) {
            if (temps_[s].size() != input.getOwned().size()) {
                temps_[s] =
        ...
        for (int batch = 0; batch < numBatches; ++batch) {
            ...
            // Phase 1: Apply twiddle factors and run sub-IFFTs
            IpplTimings::startTimer(subIFFTTimer);
