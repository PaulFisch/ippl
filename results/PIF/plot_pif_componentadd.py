...
def get_component_sum(timing_data, components=None):
    """
    Compute the sum of component times from timing data.

    Args:
        timing_data: Dict with 'timers' containing component timing info
        components: List of component names to sum (defaults to KEY_COMPONENTS)

    Returns:
        Sum of average times for the specified components
    """
    ...
    for comp in components:
        if comp in timing_data['timers']:
            total += timing_data['timers'][comp]['avg']
    return total
...
def plot_strong_scaling(all_data, output_prefix, grid_filter='512'):
    ...
    Uses sum of KEY_COMPONENTS instead of mainTimer for timing.
    """
    setup_style()
    ...
    for ax, tol in zip(axes, tolerances):
        ...
        for cluster in sorted(all_data.keys()):
            ...
            # Extract data points - use sum of components instead of wall_tot
            nodes = sorted(config_data.keys())
            times = [get_component_sum(config_data[n]) for n in nodes]
        ...
        for cluster in all_data:
            for config_name in all_data[cluster]:
                if tol_suffix in config_name and config_name.startswith(grid_filter):
                    ...
                    all_times.extend([get_component_sum(config_data[n]) for n in config_data])
                ...
                for config_name in all_data[cluster]:
                    if tol_suffix in config_name and config_name.startswith(grid_filter):
                        ...
                        if min_nodes in config_data:
                            ref_times.append(get_component_sum(config_data[min_nodes]))
...
def plot_efficiency(all_data, output_prefix, grid_filter='512'):
    """Plot parallel efficiency comparison using sum of KEY_COMPONENTS."""
    setup_style()
    ...
    for ax, tol in zip(axes, tolerances):
        ...
        for cluster in sorted(all_data.keys()):
            ...
            # Extract data points and compute efficiency - use component sums
            nodes = sorted(config_data.keys())
            times = [get_component_sum(config_data[n]) for n in nodes]

            # Compute efficiency relative to smallest node count
            min_nodes = min(nodes)
            ref_time = get_component_sum(config_data[min_nodes])
...
def plot_component_percentage(all_data, output_prefix, grid_filter='512'):
    """
    Plot percentage breakdown of component times across all clusters.
    Shows stacked bar chart with components normalized to 100%.
    """
    setup_style()
    ...
    for ax, tol in zip(axes, tolerances):
        ...
        for cluster in sorted(all_data.keys()):
            ...
            config_data = cluster_configs[matching_config]
            cluster_data[cluster] = config_data
            all_nodes_set.update(config_data.keys())

        if not cluster_data:
            continue

        # Use first cluster's nodes for x-axis
        ...
        for cluster_idx, (cluster, config_data) in enumerate(sorted(cluster_data.items())):
            offset = (cluster_idx - n_clusters/2 + 0.5) * bar_width

            # Build percentage data for each node
            for node_idx, n in enumerate(nodes):
                if n not in config_data:
                    continue

                # Get component times and use mainTimer as total
                comp_times = {}
                for comp in KEY_COMPONENTS:
                    if comp in config_data[n]['timers']:
                        t = config_data[n]['timers'][comp]['avg']
                        comp_times[comp] = t

                total = config_data[n]['wall_tot']  # Use mainTimer
                if total == 0:
                    continue
                ...
                bottom = 0
                for comp in KEY_COMPONENTS:
                    if comp in comp_times:
                        pct = comp_times[comp] / total * 100
                        color = COMPONENT_COLORS.get(comp, '#999999')
                        label = COMPONENT_LABELS.get(comp, comp) if (node_idx == 0 and cluster_idx == 0) else None

                        ax.bar(x[node_idx] + offset, pct, bar_width * 0.9,
    ...
    plt.suptitle(f'Component Distribution ({grid_filter}³ grid)', fontsize=9, y=1.02)
    plt.tight_layout()

    plt.savefig(f'{output_prefix}_percentage.pdf', facecolor='white')
    plt.savefig(f'{output_prefix}_percentage.png', dpi=300, facecolor='white')
    print(f"Saved: {output_prefix}_percentage.pdf, {output_prefix}_percentage.png")


def plot_component_percentage_by_cluster(all_data, output_prefix, grid_filter='512'):
    """
    Plot percentage breakdown of component times for each cluster separately.
    One row per cluster, columns for tolerances.
    """
    setup_style()

    clusters = sorted(all_data.keys())
    n_clusters = len(clusters)

    if n_clusters == 0:
        return

    fig, axes = plt.subplots(n_clusters, 2, figsize=(5.5, 2.2 * n_clusters))
    ...
    for row_idx, cluster in enumerate(clusters):
        for col_idx, tol in enumerate(tolerances):
            ...
            for node_idx, n in enumerate(nodes):
                ...
                for comp in KEY_COMPONENTS:
                    if comp in comp_times:
                        pct = comp_times[comp] / total * 100
                        color = COMPONENT_COLORS.get(comp, '#999999')
                        label = COMPONENT_LABELS.get(comp, comp) if node_idx == 0 else None

                        ax.bar(x[node_idx], pct, bar_width,
                               bottom=bottom, color=color,
                               edgecolor='white', linewidth=0.3,
                               label=label)
                        bottom += pct
    ...
    plt.savefig(f'{output_prefix}_percentage_by_cluster.pdf', facecolor='white')
    plt.savefig(f'{output_prefix}_percentage_by_cluster.png', dpi=300, facecolor='white')
    print(f"Saved: {output_prefix}_percentage_by_cluster.pdf")
...
def plot_all_components_grid(all_data, output_prefix, grid_filter='512'):
    ...
    for idx, (cluster, config_name) in enumerate(configs):
        ...
        # Plot total (sum of components)
        total_times = [get_component_sum(config_data[n]) for n in nodes]
        ax.plot(nodes, total_times, marker='s', color='black',
...
def print_summary(all_data):
    ...
    for cluster in sorted(all_data.keys()):
        ...
        for config_name in sorted(all_data[cluster].keys()):
            ...
            print(f"\n  {grid}, ε = {tol}:")
            print(f"  {'Nodes':>8} {'Component Sum (s)':>18} {'Wall Time (s)':>14} {'Ranks':>8}")
            print(f"  {'-'*8} {'-'*18} {'-'*14} {'-'*8}")

            for n in nodes:
                comp_sum = get_component_sum(config_data[n])
                wall_time = config_data[n]['wall_tot']
                ranks = config_data[n]['ranks']
                print(f"  {n:>8} {comp_sum:>18.3f} {wall_time:>14.3f} {ranks:>8}")
...
def main():
    ...
    parser.add_argument('--percentage', action='store_true',
                        help='Generate percentage breakdown plots')
    ...
    if args.percentage:
        plot_component_percentage(all_data, args.output, grid_filter=args.grid)
        plot_component_percentage_by_cluster(all_data, args.output, grid_filter=args.grid)
